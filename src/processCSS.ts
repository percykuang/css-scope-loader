 ...
 * @param prefix 类名前缀
 */
function processRules(rules: CssRule[], prefix: string): void {
  rules.forEach((rule: CssRule) => {
    switch (rule.type) {
      ...
      case 'layer':
        ...
        if (rule.rules) {
          processRules(rule.rules, prefix);
        }
        break;
      ...
      case 'keyframes':
        // 处理关键帧动画
        if (rule.keyframes) {
          processRules(rule.keyframes, prefix);
        } else if (rule.rules) {
 ...
 * @returns 处理后的 CSS 内容
 */
function processCSS(content: string, prefix: string): string {
  try {
    // 确保输入内容不为空
    if (!content.trim()) {
      return '';
    ...
    if (content.match(/^\.([\w-]+)\s*{\s*}\s*$/)) {
      return content.replace(/\.([\w-]+)/g, `.${prefix}$1`);
    }
    ...
    // 递归处理所有规则
    processRules(ast.stylesheet.rules, prefix);
